...
export const isDropInventory = (type?: string) => type === 'drop' || type === 'newdrop';

export const hasInventoryItems = (inventory?: Inventory) => !!inventory?.items?.some((item) => !!item.name);

export const isItemMatch = (item: Slot, search: string, filterId = 0) => {
  if (!item.name) return true;
  if (search) {
    const s = search.toLowerCase();
    if (!item.name.toLowerCase().includes(s) && !item.metadata?.label?.toLowerCase().includes(s)) return false;
  }
  if (filterId === 1) return item.name.includes('weapon') || item.name.includes('ammo');
  if (filterId === 2) return item.name.includes('bandage') || item.name.includes('med') || item.name.includes('pill');
  if (filterId === 3) return item.name.includes('water') || item.name.includes('burger') || item.name.includes('food') || item.name.includes('drink');
  if (filterId === 4) return item.name.includes('phone') || item.name.includes('key') || item.name.includes('id');
  if (filterId === 5) return item.name.includes('shirt') || item.name.includes('pant') || item.name.includes('shoe');
  return true;
};

export const createEmptyDrop = (): Inventory => ({
  id: 'newdrop',
  type: 'newdrop',
  slots: 20,
  maxWeight: 1000000,
  label: 'GROUP INVENTORY',
  items: Array.from({ length: 20 }, (_, i) => ({ slot: i + 1 })),
});

export const settleGroupDrop = (state: State) => {
  if (!hasInventoryItems(state.dropInventory)) return;
  if (isDropInventory(state.rightInventory.type) && hasInventoryItems(state.rightInventory)) {
    state.dropInventory.items.forEach((item, i) => {
      if (item.name) state.rightInventory.items[i] = { ...item };
    });
  } else {
    state.rightInventory = {
      ...
      type: 'drop',
      label: 'DROP',
      items: state.dropInventory.items.map((item) => ({ ...item })),
    };
  }
  state.dropInventory = createEmptyDrop();
};

export const resolveInventory = (state: State, type?: string) => {
  if (type === InventoryType.PLAYER) return state.leftInventory;
  if (type === InventoryType.NEWDROP) return state.dropInventory;
  if (isDropInventory(type)) {
    return isDropInventory(state.rightInventory.type) && hasInventoryItems(state.rightInventory)
      ? state.rightInventory
      : state.dropInventory;
  }
...
export const getTargetInventory = (
  ...
  sourceInventory: resolveInventory(state, sourceType),
  targetInventory: targetType
    ? resolveInventory(state, targetType)
...
export const shouldShowRightInventory = (inventory: Inventory) => {
  if (!inventory?.id || !inventory.type) return false;
  return !isDropInventory(inventory.type);
};
