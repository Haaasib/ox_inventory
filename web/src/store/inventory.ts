...
import { createEmptyDrop } from '../helpers';
  ...
  extraReducers: (builder) => {
    builder.addMatcher(isPending, (state) => {
      ...
      state.history = {
        ...
        dropInventory: current(state.dropInventory),
      };
    });
    ...
    builder.addMatcher(isRejected, (state) => {
      if (state.history && state.history.leftInventory && state.history.rightInventory) {
        ...
        if (state.history.dropInventory) state.dropInventory = state.history.dropInventory;
      }
      state.isBusy = false;
    });
  },
...
export const selectInventoryMode = (state: RootState) => state.inventory.mode;
export const selectDropInventory = (state: RootState) => state.inventory.dropInventory;
