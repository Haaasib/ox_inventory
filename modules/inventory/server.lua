...
function Inventory.ApplyBodySlots(inv)
	if not inv?.items or not shared.bodyItems then return end

	for slot, item in pairs(inv.items) do
		local target = shared.bodyItems[item.name]
		if target and slot ~= target and not inv.items[target] then
			item.slot = target
...
function Inventory.Create(id, label, invType, slots, weight, maxWeight, owner, items, groups, dbId)
	...
	if invType == 'player' then
		Inventory.ApplyBodySlots(Inventories[self.id])
	end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if inv.player and not slot then
		local bodySlot = shared.bodyItems[item.name]
		if bodySlot then
			local slotData = inv.items[bodySlot]
			if not slotData or (item.stack and slotData.name == item.name) then
				slot = bodySlot
			end
		end
	end
