...
function client.openInventory(inv, data)
    ...
    if not cache.vehicle and uiMode ~= 'quick' then
        if inv == 'player' then
    ...
    if client.screenblur and uiMode ~= 'quick' then Utils.blurIn() end
...
local function registerCommands()
	...
	lib.addKeybind({
		name = 'invquick',
		description = 'Open quick inventory',
		defaultKey = 'Z',
		onPressed = function()
			if EnableWeaponWheel or lib.progressActive() then return end
			if invOpen then
				return client.closeInventory()
			end
			if IsNuiFocused() then return end
			return client.openInventory('quick')
		end
	})
...
RegisterNetEvent('ox_inventory:createDrop', function(dropId, data, owner, slot)
	...
	if owner == cache.serverId then
		...
		if invOpen and uiMode ~= 'quick' and #(GetEntityCoords(playerPed) - data.coords) <= 1 then
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	SendNUIMessage({
		...
		data = {
			...
			bodySlots = shared.body and shared.body.slots or {}
		}
	})
	...
	client.tick = SetInterval(function()
		...
		if invOpen then
			...
			if IsDisabledControlJustPressed(0, 44) then
				SendNUIMessage({ action = 'cycleFilter', data = -1 })
